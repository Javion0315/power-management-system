...
export default {
		...
		columnsList: function () {
			...
			if (this.hasHitColumn) {
				...
				for (let dc of Object.keys(this.displayFields)) {
					let fn = dc;
					if (
						this.displayFieldsRemovePrefix &&
						fn.startsWith(this.displayFieldsRemovePrefix)
					) {
	...
	methods: {
		getThStyle(col) {
			...
			return idx !== -1
				? `min-width:${this.thStopsWidth[idx]}px;`
				: 'min-width:250px;';
		},
		...
		compositeFilterFn(rows, terms, cols, cellValue) {
			...
			if (terms && terms !== '') {
				...
				filteredRows = rows.filter(
					(row) => JSON.stringify(row).toLowerCase().indexOf(lowerTerms) !== -1
				);
			}
			return filteredRows;
		},
